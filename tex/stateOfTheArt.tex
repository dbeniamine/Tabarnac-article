...
\begin{figure}[htb]
    ...
    \caption{Screenshot from MemAxes on the example data. \tod{source? really necessary?}}
    \label{fig:memaxes}
\end{figure}

MemAxes~\cite{Gimenez14Dissecting} is one of the most advanced NUMA-oriented
visualization tool. As we can see in Figure~\ref{fig:memaxes}\tod{rm}, it allows to select a
...
tell \emph{why} it occurs, the user still has to correlate the information to
guess the source of the problem.\tod{unclear}
