...
\begin{figure*}[htb]
    \centering
    \subfigure[Structures size (pages).]{
        \includegraphics[width=.23\linewidth]{example_sz}
        \label{fig:example_sz}
    }
    \subfigure[Number of accesses.]{
        \includegraphics[width=.23\linewidth]{example_rw}
        \label{fig:example_rw}
    }
    \subfigure[Access distribution.% inside a structure.
%    Each point represents the number of access to one page done by one thread.
    ]{
        \includegraphics[width=.23\linewidth]{example_dist}
...
%   Each point shows which thread is responsible of the first access on one page.
        ...
        \includegraphics[width=.23\linewidth]{example_ft}
        \label{fig:example_ft}
    }
    \label{fig:example_plot1}
    \caption{Example plots from \TABARNAC, Figures~\ref{fig:example_dist}
    and~\ref{fig:example_ft} represents distributions inside one structure.}
\end{figure*}
