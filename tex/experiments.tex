...
We executed \TABARNAC on all benchmarks written in \texttt{C} from the OpenMP
implementation of the NAS Parallel Benchmark suite~(NPB)~\cite{Jin1999}\tod{there are just 2...}. Most
...
the access distribution \tod{rm:(as the do a fake run before the actual
...
in pages in the middle of the structure (from $500$ to $1500$), and those pages
...
\begin{figure}[htb]
    \centering
    \includegraphics[scale=.62]{is_exectime}
...
\begin{figure}[htb]
    \centering
    \includegraphics[scale=.6]{tool-ovh.pdf}
