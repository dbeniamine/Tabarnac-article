...
number of threads. Although this implementation is known to be bad, it is a
good example of non NUMA aware code, and comparing \TABARNAC~ visualization
for this algorithm to the one obtained with a better version helps understaning
how to interpret it.
Moreover when an application have such a bad memory access pattern, it
...
For the naive matrix multiplication, as we can see in figure
...
are interleaved, threads $T0$ and $T1$ works on the same pages while threads
...
improve the access distribution (here going from the naive to the bloc version),
or at least \TABARNAC~ will help choosing the best NUMA page mapping policy.

\subsection{IS}
\label{sec:exp-is}

According to the NAS parallel benchmark
website\footnote{\url{http://www.nas.nasa.gov/publications/npb.html}, visited
2015-04-25} \emph{IS} has a random memory access. Thus it is a good candidate to be
...
Figure~\ref{fig:is-res} shows the speedup of \emph{IS} in class D compared to
...
\begin{figure}[htb]
    ...
    \subfigure[Access distribution for \texttt{fz}.]{
        \includegraphics[height=.245\textheight] {ondes3d_fz_dist_orig}
        \label{fig:ondes3d-behaviour-fz-orig}
    }
    \subfigure[First-touch \texttt{fz}.]{
        \includegraphics[height=.245\textheight] {ondes3d_fz_ft_orig}
...
\begin{figure}[htb]
    ...
    \subfigure[Access distribution for \texttt{fz}.]{
        \includegraphics[height=.245\textheight] {ondes3d_fz_dist_modif}
        \label{fig:ondes3d-behaviour-fz-modif}
    }
    \subfigure[First-touch \texttt{fz}.]{
        \includegraphics[height=.245\textheight] {ondes3d_fz_ft_modif}
...
\begin{figure}[htpb]
    \centering
    \includegraphics[width=.8\linewidth]{ondes3d_exectime}
