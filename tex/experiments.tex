...
matrices of size $4096 \times 4096$ doubles for the naive version and $8192
\times 8192$ for the block, resulting on a memory usage of $128$Mib per matrix
for the naive and $512Mib$ for the block. \emph{IS} was executed
...
We use bigger matrix for the block version than for the naive one as the
execution time is $10$ times smaller for the same size.
