...
\begin{figure}[htb]
    \centering
    \includegraphics[scale=.6]{ondes3d_exectime}
...
(Interleave or the modified code) allows us to get almost $60\%$ of speedup.
...
therefore some optimization opportunities are lost. This is a case where static mapping can be substantially better than automated
...
We executed \TABARNAC on several benchmarks %written in \texttt{C}
from the OpenMP
...
the access distribution. Still the analysis of \emph{IS} caught our attention.
...
\begin{figure}[htb]
    \centering
    \includegraphics[scale=.68]{is_exectime}
...
\begin{figure}[htb]
    \centering
    \includegraphics[scale=.68]{tool-ovh.pdf}
